...
int main (int numArguments, char ** arguments){
  ...
  if (Jupiter_Sun_Earth_fixed){
    // Data accessed from ssd.jpl.nasa.gov/horizons.cgi, Time: 2020-Oct-23 00:00:00.0000 TBD
    CelestialBody &Sun = my_system.CreateBody("Sun", vec3(0, 0, 0), vec3(0, 0, 0)*yr, 1.0);
  ...
  if (Jupiter_Sun_Earth){
    // Data accessed from ssd.jpl.nasa.gov/horizons.cgi, Time: 2020-Oct-23 00:00:00.0000 TBD, except Sun velocity, which is calculated
    CelestialBody &Earth = my_system.CreateBody("Earth" ,vec3(8.575e-1, 5.002e-1, 6.633e-5), vec3(-8.833e-3, 1.486e-2, 1.132e-7)*yr, M_Earth/M_Sun);
    CelestialBody &Jupiter = my_system.CreateBody("Jupiter", vec3(2.601, -4.399, -3.995e-2), vec3(6.402e-3, 4.198e-3, -1.606e-4)*yr, M_Jupiter/M_Sun);
    CelestialBody &Sun = my_system.CreateBody("Sun", vec3(-6.158e-3, 6.384e-3, 9.046e-5), Earth.Momentum()+Jupiter.Momentum(), 1.0);
    Sun.Fix(false);
  }
